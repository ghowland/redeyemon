...
"""
Web: Render: Database Instance Info
"""
...
def Execute(data, state):
  ...
  if 'id' not in data:
    return '"id" not specified in args.  No Database Instance specified.'
  else:
    instance_id = int(data['id'])
  
  body = ''
  
  body += site_control.WebRender_DatabaseInstance(instance_id)
  body += '<br>\n'
  
  # Format the page data
  instance = site_control.RpcGetDatabaseInstance(instance_id)
  if instance:
    format_data = {'page_body':body, 'page_title':'Database Instance: %s' % instance['db_name']}
  else:
