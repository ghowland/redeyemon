...
def Execute(data=None, state=None):
  ...
  # Figure out how many volumes we need
  #NOTE(g): This is the purpose of this script...
  #TODO(g): More variations will go here in more complex provisioning scripts.
  required_volume_count = 1
  log('Provisioning storage: %s: Volumes: %d' % (storage['name'], required_volume_count))
  
  # Provision the volumes.  All the rest of it is generic, and the 'Configure'
  #   function stack script list does the heavy lifting.  This handles Volumes.
  site_control.StorageVolumeProvisionGeneric(storage_id, required_volume_count)
